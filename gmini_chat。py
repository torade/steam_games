...
def generate_ai_report(library_data):
    """Produce a detailed AI-generated report about the user's library."""
    library_text = build_library_text(library_data)

    prompt = f"""
You are an AI game analyst. Produce a **full Steam Library Report** containing:

1. **Overview**
   - Total games and general characteristics
   - Most common genres
   - What this says about the user's taste

2. **Playstyle Analysis**
   - Preferred themes, difficulty, pacing, and game structure
   - Solo vs multiplayer preference
   - Casual vs hardcore tendencies

3. **Personalized Recommendations**
   Recommend 5–8 games from **their own library** for:
   - Short play sessions (<30 minutes)
   - Relaxing / cozy mood
   - Competitive or high-focus mood
   - A full evening session

   For each recommendation:
   - Explain why it fits the category
   - Reference genres & categories

4. **Hidden Gems**
   Find 3 lesser-played or overlooked games in their library worth revisiting.

5. **Tonight’s Pick**
   Recommend one perfect game to play tonight, with reasons.

Here is the Steam library data:
...
if __name__ == "__main__":
    ...
    print("\n=== AI Steam Library Report ===\n")
    report = generate_ai_report(lib)
    print(report)
